...
try:
    ...
    pass
# from mamba_cpu import BiMambaCPU # We assume Student runs on GPU for now or reused CPU code if needed. 
# Providing UniMambaCPU is overkill unless GPU fails again.
# Let's assume GPU works as per user.
...
def main():
    print("🚀 Starting Anomaly Detection Evaluation (Student - UniMamba)")
    
    # 1. Load Model
    model = UniMambaEncoder(d_model=Config.D_MODEL).to(Config.DEVICE)
    # Load Weights
    weight_path = f"{Config.CHECKPOINT_DIR}/epoch_1.pth" # User wanted 1 epoch check? Or latest?
    # User said: "do one epoch jsut ... and also tehn find the unsuperoiced test of it"
    # So we load epoch_1.pth
