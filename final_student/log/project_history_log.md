...
| **`telegram/`** | **Notification Utility**. `telegram_notifier.py` + `config.json`. |
...
### ✅ Phase 1 Verification (Unsupervised Anomaly Detection)
**Method:** Calculated "Max Cosine Similarity" of 10k Eval flows against the Benign Training Set.
**Logic:** If the model understands "Benign", then Benign flows should have High Similarity (~1.0), and Attacks/Anomalies should have Lower Similarity.

| Metric | Score | Note |
| :--- | :--- | :--- |
| **Benign Similarity** | **0.9998** | Perfect reconstruction of normal traffic. |
| **Attack Similarity** | **0.9759** | Distinctly lower than Benign. |
| **ROC - AUC** | **97.02%** | **SOTA Level** for Unsupervised Detection. |

**Conclusion:** The model has successfully learned the latent structure of network traffic without ever seeing an attack.

### 🎯 Phase 2: Few-Shot Fine-Tuning (0.1% Data) - Attempt 2
**User Request:** Fine-tune with **Class Weighted Loss** ("Punishment") to fix imbalance.
**Settings:**
*   **Data:** 0.1% of `finetune_mixed.pkl`.
*   **Method:** CrossEntropyLoss with Inverse Class Frequencies.
*   **Objective:** Beat the Unsupervised Baseline (97%) and improve F1.

### ✅ Phase 2 Results (Few-Shot 0.1% + Weighted Loss)
The model was fine-tuned on **1,635 flows** with Class Weighted Loss (Punishment for missing attacks).

| Metric | Score | Note |
| :--- | :--- | :--- |
| **Accuracy** | **98.90%** | **Significant Improvement** (+2.2%). |
| **AUC Score** | **98.44%** | Extremely high confidence ranking. |
| **F1 Score** | **81.90%** | **Huge Jump** from 61% (Weighted Loss worked). |
| **Recall** | **86.00%** | Misses very few attacks. |
| **FPR** | **0.71%** | False alarms reduced to <1%. |
| **Latency** | **0.064 ms** | Consistent speed. |

**Conclusion:** The "Punishment" strategy (Inverse Class Weighting) successfully balanced the model. We achieved **99% Accuracy** and **82% F1** using only **0.1% of the data**. The Pretraining was highly effective.

### 🐣 Phase 3: The Student (Unidirectional Mamba)
**User Request:** Replicate the entire pipeline for a **Unidirectional** Mamba ("Student").
**Goal:** Prove that a faster/lighter model can still work, utilizing the lessons from Phase 2.
**Experiment Plan:**
1.  **Architecture:** `UniMambaEncoder` (Forward Only).
2.  **Pretraining:** 1 Epoch on Benign Data (Fast).
3.  **Unsupervised Test:** Check if it learns "Normality" without seeing the future.
4.  **Fine-Tuning:** 30 Epochs on 0.1% Data (Weighted Loss).
**Hypothesis:** Latency will be 2x faster, but AUC/Accuracy might drop slightly compared to Bi-Mamba.

**Status:** Setting up `final_student/`...
